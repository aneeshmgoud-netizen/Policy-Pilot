...
// v3 changes from v2 (see the golden-dataset eval run that motivated this):
// v2 fixed the three failures v1 had (DENY-instead-of-ESCALATE on missing
// evidence x2, and missing citations on the prompt-injection case), but its
// rule 2 ("unmet precondition -> ESCALATE") over-fired: it started
// escalating a clearly-permitted standard request (GD-01) that mentions only
// routine single-level manager approval, and left a redundant-entitlement
// DENY (GD-06, driven by rule 0, not policy text) without a grounding
// citation. v3 keeps every v2 rule and adds:
//   1. An explicit carve-out distinguishing "routine manager approval" (the
//      ordinary human-in-the-loop step that happens after every
//      recommendation regardless of decision, and therefore never itself a
//      reason to escalate) from a "named secondary/additional approving
//      authority" (a real unmet precondition, e.g. a Data Governance Owner,
//      CISO sign-off, or director sponsorship distinct from the requester's
//      own manager).
//   2. An explicit instruction that a rule-0 snapshot-driven decision still
//      requires citing any retrieved excerpt describing the underlying
//      entitlement's access protocol, if one was retrieved — grounding
//      shouldn't be skipped just because the deciding fact came from the
//      snapshot rather than the policy text.
export const SYSTEM_PROMPT_V3 = `You are an access-review policy analyst supporting a STRICT human-in-the-loop approval workflow at an enterprise. You do NOT grant, provision, or execute access. You produce a single structured RECOMMENDATION that a human reviewer will independently verify and act upon. Your recommendation never directly mutates any system.
...
3. EXPLICIT PERMISSION -> APPROVE. Choose APPROVE when a POLICY EXCERPT explicitly permits this entitlement for a requester in this cost center / department and rule 0 did not already require DENY and rule 2 did not already require ESCALATE. Ordinary, routine preconditions (standard single-level manager approval, standard recertification cadence) do not block APPROVE — only an unmet named-secondary-approval precondition (rule 2) or an explicit prohibition (rule 1) does.
...
export function buildUserMessageV3(input: RecommendationInput): string {
...
export function buildMessagesV3(input: RecommendationInput): ChatMessage[] {
  return [
    { role: 'system', content: SYSTEM_PROMPT_V3 },
    { role: 'user', content: buildUserMessageV3(input) },
  ];
}
